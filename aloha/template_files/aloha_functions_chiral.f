...
C %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    
      SUBROUTINE INPRODANG(P1,P2,INPROD)
C
C     EB: Subroutine for calculating inner product <P1||P2>. 
C      Taken from aloha_functions.f 
C      It takes two momenta, P1 and P2, and returns the inner product <P1||P2>.
C      
      IMPLICIT NONE
      DOUBLE COMPLEX P1BRAAN(2), P2KETAN(2)
      DOUBLE PRECISION P1(0:3), P2(0:3), SQP10P13, SQP20P23
      DOUBLE PRECISION RZERO, RTWO
      PARAMETER(RZERO = 0.0D0, RTWO = 2.0D0)
      DOUBLE COMPLEX INPROD

C     EB: Calculating the two components of <P1|, checking so that we don't divide by zero  
C     EB: Taken from vrxxxx    
      IF(P1(1).EQ.0D0.AND.P1(2).EQ.0D0.AND.P1(3).LT.0D0) THEN
        SQP10P13 = 0d0
      ELSE
        SQP10P13 = DSQRT(MAX(P1(0)+P1(3),RZERO))
      ENDIF
      
      IF (SQP10P13.EQ.RZERO) THEN
        P1BRAAN(1) = DCMPLX(DSQRT(RTWO*P1(0)))
      ELSE
        P1BRAAN(1) = DCMPLX(P1(1),P1(2))/SQP10P13
      ENDIF
      
      P1BRAAN(2) = DCMPLX(-SQP10P13)

C     EB: Calculating the two components of |P2>, checking so that we don't divide by zero
C     EB: Taken from rxxxxx
      IF(P2(1).EQ.0D0.AND.P2(2).EQ.0D0.AND.P2(3).LT.0D0) THEN
        SQP20P23 = 0D0
      ELSE
        SQP20P23 = DSQRT(MAX(P2(0)+P2(3),RZERO))
      ENDIF
      
      P2KETAN(1) = DCMPLX(SQP20P23)
      
      IF (SQP20P23.EQ.RZERO) THEN
        P2KETAN(2) = DCMPLX(DSQRT(RTWO*P2(0)))
      ELSE 
        P2KETAN(2) = DCMPLX(P2(1),P2(2))/SQP20P23
      ENDIF
      
C     EB: Calculating the inner product  
      INPROD = P1BRAAN(1)*P2KETAN(1) + P1BRAAN(2)*P2KETAN(2)
      END
   
C %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%   
      SUBROUTINE INPRODSQR(P1,P2,INPROD)
C
C     EB: Subroutine for calculating inner product [P1||P2]. 
C      Taken from aloha_functions.f 
C      It takes two momenta, P1 and P2, and returns the inner product [P1||P2].
C      
      ...
      IMPLICIT NONE
      DOUBLE COMPLEX P1BRASQ(2), P2KETSQ(2)
      DOUBLE PRECISION P1(0:3), P2(0:3), SQP10P13, SQP20P23
      DOUBLE PRECISION RZERO, RTWO
      PARAMETER(RZERO = 0.0D0, RTWO = 2.0D0)
      DOUBLE COMPLEX INPROD
...
C     EB: Taken from lxxxxx
      ...
      P1BRASQ(1) = DCMPLX(SQP10P13)
      
      IF(SQP10P13.EQ.RZERO) THEN
        P1BRASQ(2) = DCMPLX(DSQRT(RTWO*P1(0)))
      ELSE
        P1BRASQ(2) = DCMPLX(P1(1),-P1(2))/SQP10P13
      ENDIF
      
C     EB: Calculating the two components of |P2], checking so that we don't divide by zero
C     EB: Taken from vlxxxx     
      IF(P2(1).EQ.0D0.AND.P2(2).EQ.0D0.AND.P2(3).LT.0D0) THEN
        SQP20P23 = 0D0
      ELSE
        SQP20P23 = DSQRT(MAX(P2(0)+P2(3),RZERO))
      ...
      IF(SQP20P23.EQ.RZERO) THEN
        P2KETSQ(1) = DCMPLX(DSQRT(RTWO*P2(0)))
      ELSE
        P2KETSQ(1) = DCMPLX(P2(1),-1*P2(2))/SQP20P23
      ENDIF
      
      P2KETSQ(2) = DCMPLX(-SQP20P23)

C     EB: Calculating the inner product       
      INPROD = P1BRASQ(1)*P2KETSQ(1) + P1BRASQ(2)*P2KETSQ(2)
