      SUBROUTINE LLV1_1(F2,LEF2,REF2,PF2,MOF2,V3L,LEV3L,PV3,MO3L,MO3R,
...
C     and inner product vector length of a left-handed internal fermion
C     comming from a LLV1_1 vertex, within the chirality-flow formalism.
...
C         vector bosons with this subroutine. -1 for external V3 and 
C         0 for internal V3.
      INTEGER EXTV3
...
C       external mothers.
C     V3R(L)*MANG(ABS(MO3R(L)),ABS(MOF1(K))) gives the contraction 
C     between F1 and the right-handed part of V3.
      DO K = 1, LEF1
        F1(K) = 0
        DO L = 1, LEV3R
            F1(K) = F1(K) - (MOF1(K)/ABS(MOF1(K)))*DENOM*
     & PV3(3)*SUMF2*V3R(L)*MANG(ABS(MO3R(L)),ABS(MOF1(K)))
          ENDDO
      ENDDO      
      END
      
      SUBROUTINE LLV1R_1(F2,LEF2,REF2,PF2,MOF2,V3L,LEV3L,PV3,MO3L,MO3R,
     & V3R,LEV3R,NEXTERNAL,MSQR,MANG,COUP,M1,W1,MOF1,F1,PF1,LEF1,REF1)
C     This subroutine computes the four-momentum, inner product vector,
C     and inner product vector length of a left-handed internal fermion
C     comming from a LLV1_1 vertex where the boson is external and right-
...
C     EB: Determine the lenght of F1.
      LEF1=LEF2+REF2  
      REF1=1
