      SUBROUTINE LLV1_2(F1,LEF1,REF1,PF1,MOF1,V3L,LEV3L,PV3,MO3L,MO3R,
...
C     comming from a LLV1_2 vertex, within the chirality-flow formalism. 
...
C     EB: Contract F1 with the left-handed part of V3.       
      DO I = 1, LEF1
        DO J = 1, LEV3L
          SUMF1 = SUMF1 + F1(I)*V3L(J)*MSQR(ABS(MOF1(I)),ABS(MO3L(J)))
        ENDDO
      ENDDO
...
C       external mothers.
C     V3R(L)*MANG(ABS(MO3R(L)),ABS(MOF2(K))) gives the contraction 
C     between F1 and the right-handed part of V3.
      DO K = 1, LEF2
        F2(K) = 0
        DO L = 1, LEV3R
          F2(K) = F2(K) + (MOF2(K)/ABS(MOF2(K)))*DENOM*
     & PV3(3)*SUMF1*V3R(L)*MANG(ABS(MO3R(L)),ABS(MOF2(K)))
        ENDDO
      ENDDO 
      END    
      
      SUBROUTINE LLV1R_2(F1,LEF1,REF1,PF1,MOF1,V3L,LEV3L,PV3,MO3L,MO3R,
     & V3R,LEV3R,NEXTERNAL,MSQR,MANG,COUP,M2,W2,MOF2,F2,PF2,LEF2,REF2)
C     This subroutine computes the four-momentum, inner product vector,
C     and inner product vector length of a left-handed internal fermion
C     comming from a LLV1_2 vertexwhere the boson is external and right-
