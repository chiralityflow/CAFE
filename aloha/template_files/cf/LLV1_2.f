...
C       EB: if external mother of F2 is initial.
      ...
      SUBROUTINE LLV1I_2(F1,LEF1,PF1,MOF1,V3L,LEV3L,PV3,MO3L,MO3R,V3R,LEV3R,
...
C     EB: else F1 is external.
      ELSE
        ...
        IF (LEV3L.GT.0) THEN
          DO J = 1, LEV3L
            SUMF1 = SUMF1 + V3L(J)*MSQR(ABS(MOF1(1)),ABS(MO3L(J)))
          ENDDO
        ELSE
          LEF2 = LEF2+1
          SUMF1 = MSQR(ABS(MOF1(1)),ABS(MO3L(1)))
...
C         EB: if external mother of F2 is final.
          IF (MOF2(K).GT.0) THEN
            DO L = 1, LEV3R
              F2(K) = F2(K) + DENOM*CI*PV3(3)*SUMF1*V3R(L)*
     & MANG(ABS(MO3R(L)),ABS(MOF2(K)))
            ENDDO
          ENDIF
...
C         EB: if external mother of F1 is initial. 
     ...
     & MANG(ABS(MO3R(L)),ABS(MOF2(K)))
      ...
      ELSE
        ...
        DO K = 1, LEF2
          IF (MOF2(K).GT.0) THEN
            F2(K) = DENOM*DSQRT(RTWO)*CI*PV3(3)*SUMF1*MANG(ABS(MO3R(1)),ABS(MOF2(K)))
          ELSE
            F2(K) = -DENOM*DSQRT(RTWO)*CI*PV3(3)*SUMF1*MANG(ABS(MO3R(1)),ABS(MOF2(K)))
          ENDIF
        ENDDO
